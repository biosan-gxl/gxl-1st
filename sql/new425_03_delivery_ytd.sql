...
drop temporary table if exists shujuzu.new425_city_sales_tem00;
create temporary table if not exists shujuzu.new425_city_sales_tem00
select 
...
alter table shujuzu.new425_city_sales_tem00 add index(concatid);

-- 创建临时表 new425_city_sales_tem00  放销售YTD数据
drop temporary table if exists shujuzu.new425_city_sales_tem01;
create temporary table if not exists shujuzu.new425_city_sales_tem01(
	concatid varchar(255),
	city varchar(60),
	item_code varchar(60),
	year_ smallint,
...
-- 创建存储过程提取YTD销售数据
drop procedure if exists shujuzu.n425_pro_3;
delimiter $$
create procedure shujuzu.n425_pro_3()
begin
	declare i int default 1;
	while i <= 12 do
		insert into shujuzu.new425_city_sales_tem01
		select 
...
call shujuzu.n425_pro_3();


-- 关联获取比例数据 
drop table if exists shujuzu.new425_03_delivery_ytd;
create table if not exists shujuzu.new425_03_delivery_ytd
...
left join shujuzu.new425_city_sales_tem00 as c 
on a.concatid = c.concatid
left join shujuzu.new425_city_sales_tem01 as d
