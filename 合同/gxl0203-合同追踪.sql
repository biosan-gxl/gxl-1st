#1. 更新month_YTD（合同生效后第一次发货至今的月份数），inum_person_YTD（合同生效后至今的发货人份数），date_1st列（合同生效后第一次发货的日期）
...
LEFT JOIN 
(SELECT b.strcontractid,b.content,min(b.y_mon) as date_1st_1,IF(TIMESTAMPDIFF(MONTH,min(b.y_mon),now())>a.assess_mon,
a.assess_mon,TIMESTAMPDIFF(MONTH,min(b.y_mon),now())) as month_YTD_1 #如果至今的月份数大于考核月份数，则取考核月份数
FROM contract_standad a
LEFT JOIN contract_actual b
ON a.strcontractid= b.strcontractid AND a.content=b.content
WHERE date_format(b.y_mon,'%Y-%m')>=date_format(a.strcontractstartdate,'%Y-%m')#取年月，因b.y_mon的日期均是Y-M-1，而合同起始日期是Y-M-D
GROUP BY b.strcontractid,b.content)c
ON c.strcontractid= b.strcontractid AND c.content=b.content
WHERE b.y_mon>=c.date_1st_1 AND TIMESTAMPDIFF(MONTH,c.date_1st_1,b.y_mon)<=c.month_YTD_1#在考核月份范围内对人份数的求和，避免对合同终止后的数量求和
...
#2.更新rate_reach（达标率，考核标准分类为数量）
UPDATE contract_standad a
SET rate_reach = 
CASE WHEN standard_classify='数量' 
     THEN(CASE  WHEN standard_1st is NULL THEN inum_person_YTD/(standard_total/assess_mon*month_YTD)#仅对总量有要求，用总量除以考核月份数得到平均每月应该完成的数量
		WHEN month_YTD<=12 THEN inum_person_YTD/(standard_1st/12*month_YTD)#对第一年的要求
		WHEN month_YTD>12 AND month_YTD<=24 THEN inum_person_YTD/(standard_1st+standard_2nd/12*(month_YTD-12))#将每年的要求平均到当年的每一月
...
#3.更新TSH_YTD（取inum_person_YTD相同时间段内的TSH数量）
UPDATE contract_standad a
LEFT JOIN 
...
#4.更新rate_reach（达标率，考核标准分类为筛查率），reach（是否达标）
